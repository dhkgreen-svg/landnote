...
이 문서는 과거 작업 로그가 아니라 다음 작업자가 바로 판단할 수 있는 현재 기준입니다. 트러블슈팅 및 Vercel/빌드 이슈 재발 방지 기록은 [MEMORY.md](MEMORY.md)를 함께 확인합니다. 원격 서비스 상태는 대시보드 접근 없이 추정하지 않습니다.
...
| 매칭 알고리즘 | 같은 카테고리 시 무조건 매칭 노출되도록 점수 하향(0.6->0.25) 및 수정(update) 시에도 점수 재계산 연동 | 로컬 테스트 완료 및 Git Push |
| 매물 목록 UI 개선 | 억 단위 자동 변환 및 면적(대지/건평 분리) 표기 최적화 | 로컬 테스트 완료 및 Git Push |
| 매물 상세 수정 | 기본 정보 단일/다중 선택 로직 최적화 및 권리금 양도 예외 처리 | 로컬 테스트 완료 및 Git Push |
| 매물 상세 연락처/상태 | 매도인/임대인 전화걸기/문자/카톡공유 버튼 추가 및 즉시 상태 변경 로직 적용 | 로컬 테스트 완료 및 Git Push |
| 회원가입 및 결제 | 폼 가이드라인 강화 및 첫 7일 무료 체험 시 결제 수단 등록 건너뛰기 허용 | 로컬 테스트 완료 및 Git Push |
| PWA 설치 유도 버튼 | 랜딩/대시보드에 PWA 자동 설치 및 iOS 사파리 설치 가이드 안내 모달 추가 | 로컬 테스트 완료 및 Git Push |
| AI 간편 입력 | 기존 AI 스캐너에 카카오톡/문자 텍스트 붙여넣기 모드 추가 적용 | 로컬 테스트 완료 및 Git Push |
| 관리자 초대 링크 | Admin 대시보드에 공인중개사 백지 가입용 카카오/문자 초대 링크 발송 메뉴 추가 | 로컬 테스트 완료 및 Git Push |
| 로그인/가입 개편 | 이메일 기반 가입 제거, 전화번호 기반 가입/로그인으로 전면 개편 | 로컬 테스트 완료 및 Git Push |
| Discord 봇 자동실행 | Windows 부팅 시 백그라운드에서 자동 실행되는 VBScript 구성 완료 | 로컬 테스트 완료 |
