...
## 크로스 플랫폼 및 풀스택 일관성 유지 (UI & API)

- UI/UX 변경 사항이나 기능 추가/수정 시, 단일 파일이나 단일 뷰에만 적용하고 끝내지 않는다.
- Next.js 기반의 반응형 Web/PWA 환경이므로, 모바일 앱과 데스크탑 웹 양쪽 모두에서 정상적으로 보이고 동작하는지 고려하여 적용한다.
- 프론트엔드의 기능(예: 버튼 동작, 상태 변경 등)이 변경되면 이와 연결된 백엔드 API(NestJS) 및 DB 스키마가 완벽히 대응하고 있는지 항상 체크한다.
- 매물(Listing)과 매수 고객(Inquiry) 등 유사한 도메인의 UI를 수정할 때는, 한쪽에만 적용하지 않고 양쪽 시스템 전반(전체 앱과 서버)에 일관된 로직과 디자인 패턴이 적용되도록 한다.
